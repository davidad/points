...
get '/authenticate' do
  Net::HTTP.get URI.parse("https://graph.facebook.com/oauth/access_token?client_id=#{URI.encode($app_id)}&client_secret=#{URI.encode($app_secret)}&code=#{URI.encode(params['code'])}")
